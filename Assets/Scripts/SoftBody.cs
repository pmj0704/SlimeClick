...
public class SoftBody : MonoBehaviour
{
    ...
    private void UpdateVerticies()
    {
        for(int i = 0; i < points.Length - 1; i ++)
        {
            ...
            float _colliderRadious = points[i].gameObject.GetComponent<CircleCollider2D>().radius;
            try
            {
                spriteShape.spline.SetPosition(i, (_vertex - _towardsCenter * _colliderRadious));
            }
            catch
            {
                Debug.Log("Sprite Points are too close to each other.. recalculate");
                spriteShape.spline.SetPosition(i, (_vertex - _towardsCenter * (_colliderRadious + splineOffset)));
            }
